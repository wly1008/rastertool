...
def merge_weight(sources,          # 输入栅格文件列表
    ...
    method : str or callable, optional
        ...
        - ``merged_mask`` : ndarray of bool
            Mask of invalid pixels in ``merged_data``.
            merged_data 的无效像元掩码。
            
        - ``new_mask`` : ndarray of bool
            Mask of invalid pixels in ``new_data``.
            new_data 的无效像元掩码。
    ...
    method_kwds : dict, optional
        ...
        **Average**
        ...
        wA, wB : float, optional
            Weights for ``merged_data`` and ``new_data`` respectively.
            merged_data 与 new_data 的权重系数。

            Default is ``1`` for both, which corresponds to simple averaging.
            默认均为 1，即简单平均。

    index : int, optional
        Band index to read from each input raster. Default is ``1``.
        读取的波段序号，默认第 1 波段。
    ...
    res : float or tuple of float, optional
    ...
    dst_path : str or PathLike or DatasetWriter, optional
    ...
    with ExitStack() as exit_stack:
        with dataset_opener(sources[0]) as first:
            ...
            dest_w = np.zeros((output_count, chunk.height, chunk.width), dtype=dt)
...
def compute_max_overlap_width(sources, res=None):
    """
    计算所有 sources 两两之间的最大重叠宽度（像元数）

    Parameters
    ----------
    sources : list
        rasterio datasets 或 文件路径
    res : tuple or None
        (xres, yres)，若 None 使用第一个影像的分辨率
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            # 转换为像元
            wx = overlap_w / abs(resx)
            wy = overlap_h / abs(resy)
